...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
    ...
    pte_t *ptep=NULL;
    /*core implementation 1: implementation
    ...
    */
    /*
     * core note ( the implmentation Copy on Write)
		There are 2 situlations when code comes here.
		  ...
		  2) *ptep & PTE_P == 0 & but *ptep!=0, it means this pte is a  swap entry.
		     We should add the core's results here.
     */
        if(swap_init_ok) {
            struct Page *page=NULL;
                                    ...
                                    //(4) [NOTICE]: you myabe need to update your core's implementation for core's normal execution.
        }
        else {
        ...
        if (*ptep & PTE_P) {
            ...
            if (page_ref(old_page) == 1) {
                *ptep = (*ptep & ~PTE_COW) | PTE_W;
                if (swap_init_ok) {
                    swap_map_swappable(mm, addr, old_page, 0);
                    old_page->pra_vaddr = addr;
                }
