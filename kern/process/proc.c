...
for execution, the kernel data(for management), processor state (for context switch), files(in core), etc. ucore needs to
...
alloc_proc(void) {
    ...
    if (proc != NULL) {
    //core:implementation1 implementation
    /*
     ...
     */
     //core implementation : (update core steps)
    /*
     * below fields(add in core) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
    //core:implementation2 implementation HINT:need add some code to init fs in proc_struct, ...
...
do_fork_with_entry(uint32_t clone_flags, uintptr_t stack,
    ...
    ret = -E_NO_MEM;
    //core:implementation2 implementation
    //core:implementation2 implementation  HINT:how to copy the fs in parent's proc_struct?
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
     *   alloc_proc:   create a proc struct and init fields (core:implementation1)
     *   setup_kstack: alloc pages with size KSTACKPAGE as process kernel stack
     *   copy_mm:      process "proc" duplicate OR share process "current"'s mm according clone_flags
     *                 if clone_flags & CLONE_VM, then "share" ; else "duplicate"
     *   copy_thread:  setup the trapframe on the  process's kernel stack top and
...
//load_icode_read is used by load_icode in core
static int
...
load_icode(int fd, int argc, char **kargv) {
    /* core:implementation2 implementation  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
...
// this function isn't very correct in core
static void
