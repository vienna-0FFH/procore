...
/* A uCore executable has no CRT or dynamic linker.  Keep the compiler's
 * normal ELF linker, but feed it the same static startup and libc objects
 * used by ordinary user programs. */
static int ucore_add_runtime(TCCState *s)
{
    static const char *const runtime[] = {
        ...
        UCORE_TCC_RUNTIME_DIR "/dir.o",
        UCORE_TCC_RUNTIME_DIR "/socket.o",
        UCORE_TCC_RUNTIME_DIR "/stdio.o",
        UCORE_TCC_RUNTIME_DIR "/tcc_compat.o",
        UCORE_TCC_RUNTIME_DIR "/hash.o",
        UCORE_TCC_RUNTIME_DIR "/printfmt.o",
        UCORE_TCC_RUNTIME_DIR "/rand.o",
        UCORE_TCC_RUNTIME_DIR "/string.o",
        UCORE_TCC_RUNTIME_DIR "/libtcc1.o",
        NULL
    };
    int i;
    for (i = 0; runtime[i] != NULL; i++) {
        int ret = tcc_add_file(s, runtime[i]);
        if (ret < 0) {
            cprintf("tcc: runtime load failed %s ret=%d errors=%d\n",
                    runtime[i], ret, s->nb_errors);
            return -1;
        }
    }
    return 0;
...
static void ucore_error(void *opaque, const char *message)
{
    (void)opaque;
    cprintf("tcc: %s\n", message);
}
...
redo:
    ...
    s = s1 = tcc_new();
#ifdef UCORE_TCC
    tcc_set_error_func(s, NULL, ucore_error);
#endif
...
#ifdef UCORE_TCC
    if (ret == 0 && s->output_type == TCC_OUTPUT_EXE && !s->nostdlib)
        ret = ucore_add_runtime(s);
    if (ret < 0)
        cprintf("tcc: runtime setup failed ret=%d errors=%d\n", ret, s->nb_errors);
#endif
    if (ppfp)
        s->ppfp = ppfp;
    ...
    while (0 == ret) {
        ...
        if (f->type & AFF_TYPE_LIB) {
            ...
            if (ret < 0)
                cprintf("tcc: input failed %s ret=%d errors=%d\n",
                        f->name, ret, s->nb_errors);
        }
        if (++n == s->nb_files)
            break;
    ...
    if (s->run_test) {
        ...
        if (s->output_type == TCC_OUTPUT_MEMORY) {
            ...
            if (ret < 0)
                cprintf("tcc: output failed %s ret=%d errors=%d\n",
                        s->outfile, ret, s->nb_errors);
            if (!ret && s->gen_deps)
