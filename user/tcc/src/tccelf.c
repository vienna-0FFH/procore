...
{
#ifdef UCORE_TCC
    /* uCore's command-line front end adds its freestanding startup and
     * runtime objects explicitly.  Never search for Linux libc or a host
     * libtcc1 archive while producing the in-OS ELF. */
    (void)s1;
    return;
#else
    s1->filetype = 0;
...
#if !defined(TCC_TARGET_MACHO) && !defined(UCORE_TCC)
        if (s1->output_type != TCC_OUTPUT_MEMORY)
